...
export default function Meow() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const productsResult = (await productsResponse.json()).filter(v => v.categories.some(x => x.name !== 'meow'))
  ...
  return (
    <>
      ...
      <header className='p-4 bg-blue-500 text-white flex justify-between'>
        <div>
          <a href='/' className='text-2xl font-bold'>Pet And Pals - Woof</a>
        </div>
      ...
      <div className='flex p-8 gap-12'>
        ...
        <div>
          <h1 className='text-3xl font-bold'>Dog Products</h1>
          {products.length <= 0 && (
